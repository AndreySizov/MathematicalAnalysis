...
class TopicTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let item: TopicModel = topics[indexPath.row] as! TopicModel
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        y = "\((indexPath.row+1))"
        c = indexPath.row
        performSegue(withIdentifier: "showPointVC", sender: tableView.cellForRow(at: indexPath))
        
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let destinationVC = segue.destination as! PointTableViewController
        var i=0
        while (i<mainDictionary["points"]!.count){
            print(mainDictionary["points"]![i])
            let d = mainDictionary["points"]![i] as! PointModel
            print(d.topicId! as String)
            print(y)
            let tm: TopicModel = topics[c] as! TopicModel
            if(d.topicId! as String == tm.id){
                points.add(mainDictionary["points"]![i])
            }
            i+=1
